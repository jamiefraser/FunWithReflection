...
namespace FunWithReflection
{
    ...
    {
        public static void GetServiceInstanceAndExecuteMethod(this IServiceCollection serviceCollection, IServiceProvider provider,  Type serviceType, string methodName)
        {
            Console.WriteLine($"Trying to execute the method {methodName} on type {serviceType.Name}");
            var implementation = provider.GetRequiredService(serviceType);
            Console.WriteLine($"implementation {(implementation == null ? "is" : "is not")} null");
            var method = implementation.GetType().GetMethod(methodName);
            implementation.GetType().InvokeMember(methodName, BindingFlags.InvokeMethod, Type.DefaultBinder, implementation, null);
